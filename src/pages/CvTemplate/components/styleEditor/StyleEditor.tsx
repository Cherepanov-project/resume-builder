...
import { FieldProps, StyleEditorProps } from "./types";
...
const StyleEditor_v2: React.FC<StyleEditorProps> = ({
  ...
  const handleChange = useCallback(
    (value: string | number, el: string, place_: string | null = null): void => {
      isComplex ? setNewStyleValue(place_ || "", el, value) : setNewStyleValue(place, el, value);
      updateParent();
    },
    [isComplex, setNewStyleValue, place, updateParent],
  );

  const getStyleValue = useCallback(
    (place_: string, subKey: string = ""): { value: string | number; label: string } => {
