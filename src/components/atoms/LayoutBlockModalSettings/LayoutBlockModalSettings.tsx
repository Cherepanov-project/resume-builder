...
const LayoutBlockModalSettings = () => {
    ...
    const [text, setText] = useState<string[]>(['Click here to open modal', 'Here is your modal title', 'Wow! You opened modal.'])
    const colorRef = useRef<HTMLDivElement>(null);

    const { activeElements } = useAppSellector((state) => state.layout);
    const findName = activeElements.find(el => el.name === 'LayoutBlockModal'? el: null)
    ...
    useEffect(() => {
      ...
      dispatch(setSectionStyle({i: layout, style: {backgroundColor: bgColor, border: `2px solid ${borderColor}`, color: textColor}, text:  text, wrapperStyle: { textAlign: 'center' }, 
